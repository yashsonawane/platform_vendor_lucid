...
func init() {
	android.RegisterModuleType("lucid_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = lucidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = lucidExpandVariables(ctx, dep_file)
	...
	cmd := lucidExpandVariables(ctx, String(g.properties.Cmd))
