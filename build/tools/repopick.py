...
if __name__ == '__main__':
    # Default to LucidOS Gerrit
    default_gerrit = 'http://gerrit.lucidproject.co'

    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=textwrap.dedent('''\
        repopick.py is a utility to simplify the process of cherry picking
        patches from LucidOS's Gerrit instance (or any gerrit instance of your choosing)
    ...
    for item in mergables:
        ...
        if args.gerrit == default_gerrit:
            ...
            if args.pull:
                cmd = ['git pull --no-edit lucid', item['fetch'][method]['ref']]
            else:
                cmd = ['git fetch lucid', item['fetch'][method]['ref']]
            if args.quiet:
                cmd.append('--quiet')
            else:
                print(cmd)
